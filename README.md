...
- **メッセージ送信を検知**して「かんがえ中」— 状態はセッションカードと音声で実況
- **ツール実行を実況** — 「ターミナルで作業中」「コードをカキカキ中」などツール別の実況
- **返答が来たらジャンプしてお祝い** — カードが緑チェックになり、音声・通知でも知らせる
...
- **カニはいつも同じ大きさ** — ChatGPT のデスクトップペットと同じ構造で、開いても閉じてもマスコットは 116×112 のまま。**右上のバッジ（セッション数）をクリックするとカードが上に展開**し、**カニの頭上の ˅ ボタンで閉じる**。バッジは作業中オレンジ・ふだん緑。閉じていても監視・音声実況・通知は継続
