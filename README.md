...
- **ふだんはミニ表示** — デフォルトは小さな Clawn（116×112）だけ。右上のバッジがアクティブセッション数を示し、作業中はオレンジ・ふだんは緑に変わる。ダブルクリックで展開するとカードとセリフのフル表示、もう一度ダブルクリックでミニに戻る（シングルクリックは なでる だけなので誤操作で大きくならない）。ミニ中も監視・音声実況・通知は継続
