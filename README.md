...
- **メッセージ送信を検知**して「かんがえ中」— 状態はセッションカードで実況
- **ツール実行を実況** — 「ターミナルで作業中」「コードをカキカキ中」などツール別の実況
- **返答が来たらジャンプしてお祝い** — カードが緑チェックになり、通知でも知らせる
- **セッションカード** — アクティブなセッションをカードでスタック表示（最大6枚、7件目以降は ChatGPT ペット風の「+N」チップ）。ミニカニが各セッションの気分を映す
...
- **カニはいつも同じ大きさ** — ChatGPT のデスクトップペットと同じ構造で、開いても閉じてもマスコットは 116×112 のまま。**右上のバッジ（セッション数）をクリックするとカードが上に展開**し、**カニの頭上の ˅ ボタンで閉じる**。バッジは作業中オレンジ・ふだん緑。閉じていても監視・通知は継続
...
| 操作 | 動作 |
|---|---|
| 右上のバッジをクリック | セッションカードをひらく |
| 頭上の ˅ ボタン | セッションカードをとじる |
| セッションカードをクリック | そのセッションを Claude Desktop で開く |
| クリック | なでる（よろこぶ。サイズは変わらない） |
| ダブルクリック | ひらく／とじるの切替（バッジ・˅ と同じ） |
| ドラッグ | 好きな場所に移動（位置は記憶される） |
| 右クリック / メニューバーの 🦀 | メニュー（開閉・通知設定・デモ再生・終了など） |
...
| ドキュメント | 内容 |
|---|---|
| [docs/architecture.html](docs/architecture.html) | 設計資料（アーキテクチャ図・状態マシン・UI レイアウト・AppKit 選定理由） |
| [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) | 開発ガイド（ビルド・デプロイ・フェイクセッションでの検証レシピ・アイコン再生成） |
| [docs/PITFALLS.md](docs/PITFALLS.md) | 開発でハマった点と対策（位置アンカー・zsh・ad-hoc 署名・CGEvent 検証など 12 件） |
| [docs/FEASIBILITY.md](docs/FEASIBILITY.md) | シグナル源の技術調査レポート（何が取れて何が取れないか） |
| [CHANGELOG.md](CHANGELOG.md) | 変更履歴（v0.1〜） |
| [CLAUDE.md](CLAUDE.md) | AI エージェント向け作業ガイド（正規手順と変更時の約束） |

## アンインストール

メニューバー 🦀 → 「Claude Pet を終了」→ `/Applications/ClaudePet.app` を削除。
...
Sources/ClawnPet/
├── main.swift         # エントリポイント
├── AppDelegate.swift  # ウィンドウ・ルーティング・メニューバー・タイマー・シグナル
├── PetCore.swift      # PetEvent / PetBrain（状態マシン）
├── PetView.swift      # Clawn くんの描画・アニメーション・カード・バッジ/˅
├── Watchers.swift     # TailReader / transcript・history・main.log・registry の監視
├── Notifier.swift     # 応答の通知センター連携（osascript フォールバック）
└── CDPWatcher.swift   # claude.ai Web チャット監視（opt-in）
tools/
└── render_icon.swift  # アプリアイコンをコードから描画
```
