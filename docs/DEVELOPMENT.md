...
Claude Pet をビルド・検証・デプロイするための実務ガイドです。
...
```bash
make build      # = ./build.sh → build/ClaudePet.app
make help       # ターゲット一覧
```

`build.sh` がやること:

1. `build/ClaudePet.app/Contents/{MacOS,Resources}` を作成
...
build/ClaudePet.app/Contents/MacOS/ClaudePet > /tmp/clawn.log 2>&1 &

sleep 3
kill -USR1 $(pgrep -f ClaudePet | head -1)   # PetView が自分を PNG に描き出す
open /tmp/clawn.png
```
...
```bash
# CGWindowList で bounds を読む。owner 名は「Claude Pet」（CFBundleDisplayName）
swift -e 'import CoreGraphics
let l = CGWindowListCopyWindowInfo([.optionOnScreenOnly], kCGNullWindowID) as! [[String:Any]]
for w in l where (w["kCGWindowOwnerName"] as? String) == "Claude Pet" { print(w["kCGWindowBounds"]!) }'
```
...
```bash
pkill -f ClaudePet; CLAWN_SET_AUTOSTART=on open /Applications/ClaudePet.app
...
leaks $(pgrep -f '/Applications/ClaudePet.app' | head -1) | tail -3   # リーク検査（開発ツール同梱）
```
